...
import { db, cartItemsTable, productsTable } from "../db.js";
import { eq, and } from "drizzle-orm";
import { GetCartQueryParams, AddToCartBody, RemoveFromCartParams } from "../schemas.js";
...
router.get("/cart", async (req, res) => {
  try {
    ...
    if (!parsed.success) return res.status(400).json({ error: "sessionId is required" });
...
router.delete("/cart/:itemId", async (req, res) => {
  try {
    ...
    const [item] = await db.select().from(cartItemsTable).where(eq(cartItemsTable.id, parsed.data.itemId));

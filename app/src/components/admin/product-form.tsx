...
export function ProductForm({ open, onOpenChange, product }: ProductFormProps) {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: { name: "", brand: "", price: 0, imageUrl: "", imageBackUrl: "", description: "", available: true, featured: false },
  });

  useEffect(() => {
    if (open) {
      if (product) {
        form.reset({ name: product.name, brand: product.brand, price: product.price, imageUrl: product.imageUrl || "", imageBackUrl: product.imageBackUrl || "", description: product.description || "", available: product.available, featured: product.featured || false });
      } else {
        form.reset({ name: "", brand: "", price: 0, imageUrl: "", imageBackUrl: "", description: "", available: true, featured: false });
      }
    }
  }, [open, product, form]);
  ...
  const onSubmit = (data: FormValues) => {
    const payload = { ...data, imageUrl: data.imageUrl || undefined, imageBackUrl: data.imageBackUrl || null, description: data.description || null };
    if (isEdit) {
      updateMutation.mutate({ id: product.id, data: payload }, {
        onSuccess: () => { toast({ title: "Asset updated", description: "Changes have been persisted." }); queryClient.invalidateQueries({ queryKey: getListProductsQueryKey() }); onOpenChange(false); },
        onError: () => { toast({ title: "System Error", description: "Could not persist changes.", variant: "destructive" }); }
      });
    } else {
      createMutation.mutate({ data: payload }, {
        onSuccess: () => { toast({ title: "Asset initialized", description: "New product deployed to catalog." }); queryClient.invalidateQueries({ queryKey: getListProductsQueryKey() }); onOpenChange(false); },
        onError: () => { toast({ title: "System Error", description: "Could not initialize asset.", variant: "destructive" }); }
      });
    }
  };
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 mt-2">
            <div className="grid grid-cols-2 gap-5">
              <FormField control={form.control} name="name" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Input placeholder="Cap Name" {...field} className="bg-input/50 border-border/50 focus-visible:ring-primary" /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
              <FormField control={form.control} name="brand" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Input placeholder="Brand Name" {...field} className="bg-input/50 border-border/50 focus-visible:ring-primary" /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
            </div>
            <FormField control={form.control} name="price" render={({ field }) => (
              <FormItem>
                ...
                <FormControl><Input type="number" step="0.01" {...field} className="bg-input/50 border-border/50 focus-visible:ring-primary w-[160px] font-mono" /></FormControl>
                <FormMessage />
              </FormItem>
            )} />
            <FormField control={form.control} name="description" render={({ field }) => (
              <FormItem>
                ...
                <FormControl><Textarea placeholder="Product details..." {...field} className="bg-input/50 border-border/50 focus-visible:ring-primary resize-none h-24" /></FormControl>
                <FormMessage />
              </FormItem>
            )} />
            <div className="space-y-4 border border-border/30 rounded-lg p-5 bg-background shadow-inner">
              ...
              <FormField control={form.control} name="imageUrl" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Input placeholder="https://..." {...field} className="bg-input/50 border-border/50 focus-visible:ring-primary font-mono text-sm" /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
              <FormField control={form.control} name="imageBackUrl" render={({ field }) => (
                <FormItem>

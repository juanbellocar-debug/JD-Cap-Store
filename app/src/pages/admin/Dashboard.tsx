...
function DashboardContent() {
  ...
  const handleDelete = (id: number) => {
    deleteMutation.mutate({ id }, {
      onSuccess: () => { toast({ title: "Product purged" }); queryClient.invalidateQueries({ queryKey: getListProductsQueryKey() }); },
      onError: () => { toast({ title: "System Error", description: "Failed to purge product.", variant: "destructive" }); }
    });
  };

  const handleToggle = (id: number, field: "available" | "featured", currentValue: boolean, productData: any) => {
    updateMutation.mutate({ id, data: { ...productData, [field]: !currentValue } }, {
      onSuccess: () => { toast({ title: "Status updated" }); queryClient.invalidateQueries({ queryKey: getListProductsQueryKey() }); },
      onError: () => { toast({ title: "System Error", variant: "destructive" }); }
    });
  };
  ...
  return (
    <Layout>
      <div className="flex flex-col gap-8">
        ...
        <div className="flex items-center gap-2 max-w-md">
          <div className="relative flex-1">
            ...
            <Input type="search" placeholder="Search designation or origin..." className="pl-9 bg-card border-border/50 shadow-sm focus-visible:ring-primary" value={search} onChange={(e) => setSearch(e.target.value)} />
          </div>
        </div>
                ...
                <TableRow key={product.id} className="group border-border/50 hover:bg-muted/10 transition-colors">
                  <TableCell>
                    <div className="w-12 h-12 rounded bg-background flex items-center justify-center overflow-hidden border border-border/50 shadow-sm">
                      {product.imageUrl ? <img src={product.imageUrl} alt={product.name} className="w-full h-full object-cover" /> : <Package className="w-5 h-5 text-muted-foreground/50" />}
                    </div>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell className="text-center">
                    <Switch checked={product.available} onCheckedChange={() => handleToggle(product.id, "available", product.available, product)} className="data-[state=checked]:bg-primary shadow-sm" />
                  </TableCell>
                  <TableCell className="text-center">
                    <Switch checked={product.featured || false} onCheckedChange={() => handleToggle(product.id, "featured", product.featured || false, product)} className="data-[state=checked]:bg-primary shadow-sm" />
                  </TableCell>
                  <TableCell className="text-right">
                    <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-foreground" onClick={() => { setEditingProduct(product); setIsFormOpen(true); }}>
                        ...
                        <AlertDialogContent className="dark bg-card border-border shadow-2xl">
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                              This will permanently remove <span className="font-semibold text-foreground">{product.name}</span> from the catalog.
                            </AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
                            <AlertDialogCancel className="border-border/50">Cancel</AlertDialogCancel>
                            <AlertDialogAction onClick={() => handleDelete(product.id)} className="bg-destructive text-destructive-foreground hover:bg-destructive/90 shadow-md">Execute</AlertDialogAction>
                          </AlertDialogFooter>
                        </AlertDialogContent>
                      </AlertDialog>
                    </div>
      ...
      <ProductForm open={isFormOpen} onOpenChange={setIsFormOpen} product={editingProduct} />
    </Layout>
  );
}

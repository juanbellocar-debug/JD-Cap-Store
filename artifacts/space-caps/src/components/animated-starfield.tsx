...
export function AnimatedStarfield() {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // Nebula blobs — subtle deep-space color clouds
      const nebulae = [
      ...
      for (const s of stars) {
        ...
        if (s.bright) {
          ...
          // Cross diffraction spike for the brightest
          if (s.radius > 2) {
            ctx.strokeStyle = `rgba(220,228,255,${op * 0.3})`;
            ctx.lineWidth = 0.5;
            ctx.beginPath();
            ctx.moveTo(s.x - s.radius * 8, s.y);
            ctx.lineTo(s.x + s.radius * 8, s.y);
            ctx.stroke();
            ctx.beginPath();
            ctx.moveTo(s.x, s.y - s.radius * 8);
            ctx.lineTo(s.x, s.y + s.radius * 8);
            ctx.stroke();
          }
        }
      ...
      for (let i = 0; i < shooters.length; i++) {
        ...
        if (s.x > w + 120 || s.y > h + 120) {
          shooters[i] = makeShooter(w, h);
          continue;
        }
